    ...
    <div className="flex flex-col flex-1 min-h-dvh bg-diapilot-gradient text-white relative overflow-y-auto no-scrollbar select-none pb-[max(2rem,env(safe-area-inset-bottom,0px))]">
      ...
      <div className="pt-[max(1.25rem,env(safe-area-inset-top,0px))] px-6 flex items-center justify-between z-20">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-full bg-white/20 p-0.5 flex items-center justify-center">
            <Image
        ...
        <Link
          href="/"
          locale={isRtl ? "en" : "ar"}
          className="text-xs font-bold px-3.5 py-1 rounded-full bg-white/15 hover:bg-white/25 border border-white/30 backdrop-blur-md transition-all active:scale-95 text-white"
        >
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col px-6 pt-5 pb-6 z-10">
        {/* Hero Typography */}
        <div className="flex flex-col gap-2.5 max-w-[340px]">
          ...
          {/* Liquid Glassmorphism CTA Action Button */}
          <div className="py-4">
            <Link
              href="/language"
        ...
        <div className="relative w-full max-w-[280px] sm:max-w-[310px] aspect-square mx-auto mt-12 flex items-center justify-center">
          ...
          <div className="absolute top-1 left-1/2 -translate-x-1/2 w-11 h-11 rounded-full bg-[#DCFCE7] border-2 border-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
            <AppleEmoji name="apple" size={24} />
          </div>

          {/* Orbiting Satellite 2: Right / Foot Care */}
          <div className="absolute top-1/3 -right-1 w-11 h-11 rounded-full bg-[#FED7AA] border-2 border-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
            <AppleEmoji name="foot" size={24} />
          </div>

          {/* Orbiting Satellite 3: Bottom Right / Eye Care */}
          <div className="absolute bottom-4 right-4 w-11 h-11 rounded-full bg-[#E0F2FE] border-2 border-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
            <AppleEmoji name="eye" size={24} />
          </div>

          {/* Orbiting Satellite 4: Left / Diabetes Monitoring & Meter */}
          <div className="absolute top-1/2 -left-2 -translate-y-1/2 w-11 h-11 rounded-full bg-[#FFEDD5] border-2 border-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
            <AppleEmoji name="blood" size={22} />
          </div>

          {/* Orbiting Satellite 5: Bottom Left / Insulin & Medication */}
          <div className="absolute bottom-2 left-6 w-11 h-11 rounded-full bg-[#FCE7F3] border-2 border-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
